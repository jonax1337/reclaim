<script lang="ts">
  ...
  import { Minus, Square, X, Zap } from 'lucide-svelte';
...
<div class="titlebar" data-tauri-drag-region>
  <div class="brand" data-tauri-drag-region>
    <span class="logo" aria-hidden="true"><Zap size={13} strokeWidth={2.25} fill="currentColor" /></span>
  ...
  <div class="controls">
    <button class="ctl" aria-label="Minimize" onclick={() => win.minimize()}><Minus size={14} /></button>
    <button class="ctl" aria-label="Maximize" onclick={() => win.toggleMaximize()}><Square size={12} /></button>
    <button class="ctl close" aria-label="Close" onclick={() => win.close()}><X size={14} /></button>
  </div>
</div>
